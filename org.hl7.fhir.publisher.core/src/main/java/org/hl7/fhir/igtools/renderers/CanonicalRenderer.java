...
public class CanonicalRenderer extends BaseRenderer {
  ...
  private void genSummaryCore1(StringBuilder b, Set<String> rows) {
    ...
    if (hasSummaryRow(rows, "version")) {
      if (cr.hasVersion()) {
        b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_VER))+":</td><td>"+Utilities.escapeXml(cr.getVersion())+"</td></tr>\r\n");
      } else if (cr.hasExtension("http://terminology.hl7.org/StructureDefinition/ext-namingsystem-version")) {
        b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_VER))+":</td><td>"+Utilities.escapeXml(ToolingExtensions.readStringExtension(cr, "http://terminology.hl7.org/StructureDefinition/ext-namingsystem-version"))+"</td></tr>\r\n");
      }
    }
    ...
    if (hasSummaryRow(rows, "name")) {
      ...
      b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_NAME))+":</td><td>"+Utilities.escapeXml(name)+"</td></tr>\r\n");
    }
    if (hasSummaryRow(rows, "title")) {
      if (title != null && !title.equalsIgnoreCase(name)) {
        b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_TITLE))+":</td><td>"+Utilities.escapeXml(title)+"</td></tr>\r\n");
      }
    }
    if (hasSummaryRow(rows, "status")) {
      b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_STATUS))+":</td><td>"+describeStatus(cr)+"</td></tr>\r\n");
    }
    if (hasSummaryRow(rows, "definition")) {
      if (cr.hasDescription()) {
        b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_DEFINITION))+":</td><td>"+processMarkdown("description", cr.getDescriptionElement())+"</td></tr>\r\n");
      }
    }
    if (hasSummaryRow(rows, "publisher")) {
      if (cr.hasPublisher())
        b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_PUBLISHER))+":</td><td>"+buildPublisherLinks(cr)+"</td></tr>\r\n");
    }
    if (hasSummaryRow(rows, "committee")) {
      if (cr.hasExtension(ToolingExtensions.EXT_WORKGROUP)) {
        b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_COMMITTEE))+":</td><td>"+renderCommitteeLink(cr)+"</td></tr>\r\n");
      }
    }
    if (hasSummaryRow(rows, "copyright")) {
      if (cr.hasCopyright())
        b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_COPYRIGHT))+":</td><td>"+processMarkdown("copyright", cr.getCopyrightElement())+"</td></tr>\r\n");
    }
    if (hasSummaryRow(rows, "maturity")) {
      if (ToolingExtensions.hasExtension(cr, ToolingExtensions.EXT_FMM_LEVEL)) {
        ...
        b.append(" <tr><td><a class=\"fmm\" href=\"http://hl7.org/fhir/versions.html#maturity\" title=\"Maturity Level\">"+(context.formatMessage(RenderingContext.CANON_REND_MATURITY))+"</a>:</td><td>"+ToolingExtensions.readStringExtension(cr, ToolingExtensions.EXT_FMM_LEVEL)+"</td></tr>\r\n");
      }    
    }
  }
  ...
  private void genSummaryCore2(StringBuilder b, FetchedResource r, boolean xml, boolean json, boolean ttl, Set<String> rows) {
    if (hasSummaryRow(rows, "formats")) {
      if (xml || json || ttl) {
        b.append(" <tr><td>"+(context.formatMessage(RenderingContext.CANON_REND_SOURCE_RES))+":</td><td>");
        boolean first = true;
        ...
        if (xml) {
          first = false;
          b.append("<a href=\""+igp.doReplacements(filename,  r,  null, "xml")+"\">"+(context.formatMessage(RenderingContext.CANON_REND_XML))+"</a>");
        }
        if (json) {
          if (first) first = false; else b.append(" / ");
          b.append("<a href=\""+igp.doReplacements(filename,  r,  null, "json")+"\">"+(context.formatMessage(RenderingContext.CANON_REND_JSON))+"</a>");
        }
        if (ttl) {
          if (first) first = false; else b.append(" / ");
          b.append("<a href=\""+igp.doReplacements(filename,  r,  null, "ttl")+"\">"+(context.formatMessage(RenderingContext.CANON_REND_TURTLE))+"</a>");
        }
        b.append("</td></tr>\r\n");
      }    
    }
