    ...
    if (errMsg == null && pid != null) {
      ...
      for (VersionInstance vi : versionList) {
        ...
        try {
          ...
          vi.context = new SimpleWorkerContext.SimpleWorkerContextBuilder().withTerminologyCachePath(Utilities.path(context.getTxCache(), vi.version)).fromPackage(core, new PublisherLoader(core, SpecMapManager.fromPackage(core), core.getWebLocation(), null).makeLoader());
          //vi.context.initTS();
